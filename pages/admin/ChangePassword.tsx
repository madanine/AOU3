  ...
  return (
    <div className="max-w-xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="text-center">
        <h1 className="text-3xl font-black text-gray-900 tracking-tight">{lang === 'AR' ? 'تغيير كلمة المرور' : 'Change My Password'}</h1>
        <p className="text-gray-500 font-medium mt-1">{lang === 'AR' ? 'تحديث بيانات الدخول الخاصة بك' : 'Update your personal access credentials'}</p>
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl border p-10 space-y-6" style={{ borderColor: 'var(--border-color)' }}>
        <div className="flex items-center gap-4 mb-4">
           ...
           <div>
             <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">{lang === 'AR' ? 'حساب المسؤول' : 'Admin Account'}</p>
             <h3 className="font-black text-gray-900 mt-1">{user?.fullName}</h3>
           </div>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase text-gray-400 ml-1">{lang === 'AR' ? 'كلمة المرور الحالية' : 'Current Password'}</label>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase text-gray-400 ml-1">{lang === 'AR' ? 'كلمة المرور الجديدة' : 'New Password'}</label>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300" size={18} />
              <input
                type="password"
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase text-gray-400 ml-1">{lang === 'AR' ? 'تأكيد كلمة المرور الجديدة' : 'Confirm New Password'}</label>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300" size={18} />
              <input
                type="password"
