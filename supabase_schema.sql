...
create table profiles (
  ...
  major text,
  password text,
  -- Added for fallback users
  assigned_courses text [],
  -- Array of course IDs
  supervisor_permissions jsonb,
  -- {attendance: boolean, assignments: boolean, grading: boolean}
...
create table courses (
  ...
  semester_id uuid references semesters(id) on delete
  set null,
    whatsapp_link text,
    telegram_link text,
    lecture_link text,
...
create table attendance (
  ...
  -- 0 to 11
  status boolean,
  -- true for present, false for absent, null for not taken
