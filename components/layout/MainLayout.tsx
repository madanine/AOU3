  ...
  const getMenuItems = () => {
    if (user?.role === 'admin') {
      ...
      const items = [
        ...
        { label: lang === 'AR' ? 'الحضور والمشاركة' : 'Attendance & Participation', path: '/admin/attendance', icon: CheckSquare, key: 'attendance' },
    ...
    if (user?.role === 'supervisor') {
      ...
      if (perms.attendance) {
        items.push({ label: lang === 'AR' ? 'الحضور والمشاركة' : 'Attendance & Participation', path: '/supervisor/attendance', icon: CheckSquare, key: 'attendance' });
      }
